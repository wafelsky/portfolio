...
class Navbar extends React.Component {
  constructor(props) {
    super(props);
    this.state = {
      ///////// the ID must exist in the document before you add the button!
      iColor: styles.c,
      width: "",
      navmargin: "-400px",
  ...
  smallscreenpanel() {
    this.state.navmargin === "-400px"
      ? this.setState({ navmargin: "0px" })
      : this.setState({ navmargin: "-400px" });
  }
  componentDidMount() {
    ...
    document.addEventListener("scroll", () => {
      ...
      for (let i = 0; i < this.props.navbuttons.length; i++) {
        let local = document
          .getElementById(this.props.navbuttons[i][0])
          .getBoundingClientRect().y;
        if (local < 10) {
          this.setState({ currentlyclicked: this.props.navbuttons[i][0] });
        }
      }
    });
  }
    ...
    return (
      <div className="absolute-main-nav" id="Home">
        ...
        <div
          ...
          style={{
            backgroundColor:
              this.state.windowoffset !== 0 ? "#191919" : "#070707",
            border:
              this.state.windowoffset !== 0 ? "2px solid #101010" : "none",
          }}
        >
                    ...
                    <div key={i} className="button-wrapper">
                      ...
                      >
                        <div
                          ...
                          onClick={() => {
                            this.setState({
                              currentlyclicked: this.props.navbuttons[i][0],
                            });
                          }}
                        >
                          <h1
                            className="left-panel-font"
                            style={{
                              color:
                                this.state.currentlyclicked ===
                                this.props.navbuttons[i][0]
                                  ? styles.cfour
                                  : "white",
                            }}
                          >
          ...
          {this.state.width < this.state.widthchange && (
            <div
              className="hide-panel-overflow"
              style={{
                height: this.state.navmargin === "-400px" ? "0px" : "320px",
              }}
            >
              <div className="smaller-screen-panel " style={scrollnav}>
                {this.props.navbuttons.map((x, i) => {
                  return (
                    <div key={i} className="button-wrapper-right">
                      <div
                      ...
                      >
                        <h1 className="left-panel-font">
                          <a
                            ...
                            style={{
                              color:
                                this.state.currentlyclicked === "home"
                                  ? this.state.iColor
                                  : "white",
                              textAlign: "center",
                            }}
                      ...
                      <div
                        className="underline-magic-right"
                        style={{
                          width:
                            this.state.currentlyclicked ===
                            this.props.navbuttons[i][0]
        ...
        {this.action === "copy" && (
          <div>
            <i
              ...
              className={`${this.icon} hover-icon`}
            ></i>
