...
handleSubmit (event) {
  ...
  */
  ...
  this.state.messegearray.push([[this.state.feedback],["20px"],[0],"start"])
  }
 ...
 <div className="round-background"></div>
   ...
   <i className="fas fa-smile-wink"></i>
    ...
    <form className="name">
        ...
        {/* 
        <textarea  className="textarea-class-2"	id="name" 	name="name" 	onChange={this.handleChange2}	required	value={this.state.name}	/>
   */}
      <div  className="mesege-contents">
      <h1 className="textarea-headlines"></h1>
        <textarea className="textarea-class-1" id="test-mailing"	name="test-mailing"	onChange={this.handleChange}placeholder="" required	value={this.state.feedback}/>
        </div>
      <div className="reveletion-button">

      
      <div type="button" value="Submit" className="submit-button" onClick={this.handleSubmit}><i className="fas fa-arrow-circle-right"></i></div>
    ...
    <div className="time-chat">{this.state.hours}:{this.state.minutes<10 && 0}{this.state.minutes}</div>

    <div className="icon-panel">
    <div className="connection"><i className="fas fa-wifi"></i></div>
    <div className="signal"><i className="fas fa-signal"></i></div>
    <div className="battery"><i className="fas fa-battery-three-quarters"></i></div>
    </div>
    <div className="myselfchat"> <img src={myself} className="myself-img"></img></div>
  ...
  {/* <div className="thepointer-message"></div>*/}
    ...
    <h2 className="mesge-to-viewers">Hello stranger! Please remember to leave contact information. </h2>
    </div>

    <div>
    {this.state.messegearray.map((value, i) => {
      return <div key={i} className="user-message-cont" style={{marginLeft:this.state.messegearray[i][1]}}>
       <h2 className="user-message" >{this.state.messegearray[i][0]}</h2>

       {this.state.messegearray[i][2] === 0 &&  <h1 className="succes-sent">Sent!</h1>}
